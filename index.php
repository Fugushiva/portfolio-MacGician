<?php 
    include('include/header.php');
?>
<article class="article">
    <div id="slogan"><quote>Dream until your dreams come true...</quote>
    <br>
    <i id="slogAuth">-Aerosmith</i>
    </div>
    <div id="aboutMe">
        <p>
            Je vois l'art musical comme un langage a part entiere, une forme de communication qui permettrais de transmettre ce que l'on ressent. 
            Les émotions sont souvent difficiles a exprimer par des mots, la musique me sert donc à partager ces émotions aux gens grâce à ses notes, accords et harmonies. 
        </p>   
        <p> 
            La chaleur que peut transmettre certains instruments, les moments de tensions ainsi que les moments de vides se représentent naturellement dans une composition faites pour transmettre des choses. 
            La musique est un art, tout comme le peintre utilise le pinceau et les couleurs pour faire voyager la personne qui regarde son œuvre, j'utilise les notes afin de faire vivre a mes auditeurs une histoire leurs racontant un voyage vers leurs émotions
        </p>
        <p>
            Je suis déterminé à poursuivre ma passion pour la musique et à élargir mes horizons artistiques. 
            Mon objectif ultime est de travailler sur des projets ambitieux tels que des films, des séries et des mangas, où ma musique pourra contribuer à créer des expériences cinématographiques et narratives inoubliables.
        </p>
        <p>
            Je rêve de composer des bandes originales qui transportent les spectateurs dans des mondes imaginaires, où chaque note musicale évoque des émotions profondes et renforce l'impact des images à l'écran.
            Je crois fermement que la musique a le pouvoir de transcender les frontières linguistiques et culturelles, et je veux exploiter ce pouvoir pour raconter des histoires à travers des notes et des mélodies.
        </p>
    </div>
</article>



<?php 
    include("include/footer.php");
?>